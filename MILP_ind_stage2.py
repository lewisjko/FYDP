...
for LP_4 in [LP_eps_4, LP_cost_4]:
    for i, h in enumerate([str(i) for i in input_df.index]):
        ...
        if h == '0': #at hour zero, no accumulatione exists
            #LP_4 += I_H2_4[h] == H2_tank_in_4[h] - H2_tank_out_4[h]
            LP_4 += I_H2_4[h] >= Imin * N_tank_4 + H2_tank_in_4[h] - H2_tank_out_4[h]
            LP_4 += I_H2_4[h] <= Imax * N_tank_4 + H2_tank_in_4[h] - H2_tank_out_4[h]
...
# Objectives
LP_eps_4 += em_before - em_compressor_4 - em_electrolyzer_4
LP_eps_4.solve()

offset_max_4 = LP_eps_4.objective.value()

LP_cost_4 += em_before - em_compressor_4 - em_electrolyzer_4 == em_offset_4
LP_cost_4 += em_offset_4 >= phi * em_offset_max_4
LP_cost_4 += CAPEX_4 + OPEX_4 * TVM, 'Cost_4'

LP_cost_4.solve()
print([x.varValue for x in LP_cost_4.variables()])
print(pulp.value(LP_cost_4.objective))